...
object UdeskMPermissionUtils {
    ...
    private fun getContext(`object`: Any): Context {
        return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
            when (`object`) {
                is android.app.Fragment -> `object`.activity
                is androidx.fragment.app.Fragment -> UdeskBaseApp.get()
    ...
    private fun checkCallingObjectSuitability(`object`: Any?) {
        ...
        val isAppFragment = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
